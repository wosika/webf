...
// Convert legacy AllowedRulesType enum to new AllowedRules class
AllowedRules CSSParserImpl::ConvertToAllowedRules(AllowedRulesType legacy_type) {
  switch (legacy_type) {
    case kAllowCharsetRules:
      return kTopLevelRules;
    case kAllowLayerStatementRules:
      return kTopLevelRules;
    case kAllowImportRulesType:
      return kTopLevelRules;
    case kAllowNamespaceRules:
      return kTopLevelRules;
    case kRegularRulesType:
      return CSSParserImpl::kRegularRules;
    case kKeyframeRulesType:
      return CSSParserImpl::kKeyframeRules;
    case kFontFeatureRulesType:
      return CSSParserImpl::kFontFeatureRules;
    case kNoRulesType:
      return CSSParserImpl::kNoRules;
    case kNestedGroupRulesType:
      return CSSParserImpl::kNestedGroupRules;
    case kPageMarginRulesType:
...
// Helper to check if AllowedRulesType equals AllowedRules
static bool AreRulesEqual(CSSParserImpl::AllowedRulesType legacy, const AllowedRules& rules) {
  return CSSParserImpl::ConvertToAllowedRules(legacy) == rules;
}

namespace {

// This may still consume tokens if it fails
String ConsumeStringOrURI(CSSParserTokenStream& stream) {
  const CSSParserToken& token = stream.Peek();

  if (token.GetType() == kStringToken || token.GetType() == kUrlToken) {
    return String(stream.ConsumeIncludingWhitespace().Value());
  }
...
std::shared_ptr<StyleRuleBase> CSSParserImpl::ParseRule(const String& string,
                                                        std::shared_ptr<const CSSParserContext> context,
  ...
  if (stream.UncheckedPeek().GetType() == kAtKeywordToken) {
    rule = parser.ConsumeAtRule(stream, allowed_rules, CSSNestingType::kNone,
                                /*parent_rule_for_nesting=*/nullptr);
  } else if (allowed_rules == kPageMarginRulesType) {
...
std::shared_ptr<StyleRuleBase> CSSParserImpl::ParseRule(const String& string,
  ...
  // Convert AllowedRules to AllowedRulesType and delegate to existing implementation
  AllowedRulesType legacy_type = kAllowImportRulesType;  // Default
  
  // Check which legacy type corresponds to the AllowedRules
  if (allowed_rules == kAllRules) {
    legacy_type = kAllowImportRulesType;
  } else if (allowed_rules == kRegularRules) {
    legacy_type = kRegularRulesType;
  } else if (allowed_rules == kKeyframeRules) {
    legacy_type = kKeyframeRulesType;
  } else if (allowed_rules == kFontFeatureRules) {
    legacy_type = kFontFeatureRulesType;
  } else if (allowed_rules == kPageMarginRules) {
    legacy_type = kPageMarginRulesType;
  } else if (allowed_rules == kLayerRules) {
    legacy_type = kLayerRulesType;
  } else if (allowed_rules == kPageRules) {
    legacy_type = kPageRulesType;
  } else if (allowed_rules == kStyleRules) {
    legacy_type = kStyleRulesType;
  }
  
  return ParseRule(string, context, nesting_type, parent_rule_for_nesting, style_sheet, legacy_type);
}

ParseSheetResult CSSParserImpl::ParseStyleSheet(const String& string,
...
std::shared_ptr<const CSSSelectorList> CSSParserImpl::ParsePageSelector(
    ...
    std::shared_ptr<StyleSheetContents> style_sheet,
    std::shared_ptr<const CSSParserContext> context) {
  // Convert to stream and call the stream version
  CSSTokenizer tokenizer(range.Serialize());
  CSSParserTokenStream stream(tokenizer);
  return ParsePageSelector(stream, style_sheet, context);
}
...
std::shared_ptr<const CSSSelectorList> CSSParserImpl::ParsePageSelector(
  ...
  if (stream.Peek().GetType() == kIdentToken) {
    type_selector = stream.ConsumeIncludingWhitespace().Value().ToAtomicString();
  }
  ...
  if (stream.Peek().GetType() == kColonToken) {
    ...
    pseudo = stream.ConsumeIncludingWhitespace().Value().ToAtomicString();
  }
...
std::unique_ptr<std::vector<KeyframeOffset>> CSSParserImpl::ParseKeyframeKeyList(
  ...
  CSSParserTokenStream stream(tokenizer);
  return ConsumeKeyframeKeyList(std::move(context), stream);
}
...
static CSSParserImpl::AllowedRulesType ComputeNewAllowedRules(CSSParserImpl::AllowedRulesType allowed_rules,
  ...
  if (!rule || allowed_rules == CSSParserImpl::kKeyframeRulesType || allowed_rules == CSSParserImpl::kFontFeatureRulesType ||
      allowed_rules == CSSParserImpl::kNoRulesType) {
    return allowed_rules;
  }
  assert(allowed_rules <= CSSParserImpl::kRegularRulesType);
  if (rule->IsCharsetRule()) {
  ...
  if (rule->IsLayerStatementRule()) {
    ...
    return CSSParserImpl::kRegularRulesType;
  }
  if (rule->IsImportRule()) {
    return CSSParserImpl::kAllowImportRulesType;
  }
...
bool CSSParserImpl::ConsumeRuleList(CSSParserTokenStream& stream,
                                    ...
                                    const T callback) {
  AllowedRulesType allowed_rules = kRegularRulesType;
  switch (rule_list_type) {
    case kTopLevelRuleList:
      allowed_rules = kAllowCharsetRules;
      break;
    case kRegularRuleList:
      allowed_rules = kRegularRulesType;
      break;
    case kKeyframesRuleList:
      allowed_rules = kKeyframeRulesType;
      break;
    case kFontFeatureRuleList:
      allowed_rules = kFontFeatureRulesType;
      break;
    default:
...
std::shared_ptr<StyleRuleBase> CSSParserImpl::ConsumeNestedRule(
  ...
  if (!id.has_value()) {
    ...
    child =
        ConsumeAtRuleContents(*id, stream, parent_rule_type == StyleRule::kPage ? kPageMarginRulesType : kNestedGroupRulesType,
...
std::shared_ptr<StyleRuleKeyframe> CSSParserImpl::ConsumeKeyframeStyleRule(
  ...
  // Convert prelude range to stream for processing
  CSSTokenizer tokenizer(prelude.Serialize());
  CSSParserTokenStream prelude_stream(tokenizer);
  std::unique_ptr<std::vector<KeyframeOffset>> key_list = ConsumeKeyframeKeyList(context_, prelude_stream);
  if (!key_list) {
    return nullptr;
  }
...
// Helper to check if at-rule prelude would be empty without consuming
bool IsAtRulePreludeEmpty(CSSParserTokenStream& stream) {
  stream.EnsureLookAhead();
  return stream.Peek().GetType() == kLeftBraceToken || 
...
static CSSParserTokenOffsets ReparseForOffsets(StringView prelude, const CSSParserTokenRange range) {
  std::vector<size_t> raw_offsets = CSSTokenizer(prelude).TokenizeToEOFWithOffsets().second;
  raw_offsets.reserve(32);
  return {range.RemainingSpan(), std::move(raw_offsets), prelude};
}

std::shared_ptr<const MediaQuerySet> CSSParserImpl::CachedMediaQuerySet(const String& prelude_string,
                                                                        webf::CSSParserTokenRange prelude,
...
std::shared_ptr<StyleRuleFontFace> CSSParserImpl::ConsumeFontFaceRule(CSSParserTokenStream& stream) {
  ...
  // Check if prelude is empty before consuming
  if (!IsAtRulePreludeEmpty(stream)) {
    return nullptr;  // Parse error; @font-face prelude should be empty
  }
  
  // Consume the empty prelude
...
std::shared_ptr<StyleRuleCounterStyle> CSSParserImpl::ConsumeCounterStyleRule(CSSParserTokenStream& stream) {
  ...
  // Check if prelude is non-empty first
  stream.ConsumeWhitespace();
  if (IsAtRulePreludeEmpty(stream)) {
    return nullptr;  // Parse error; prelude should contain counter-style name
  }
  
  // Get counter-style name from stream
  const CSSParserToken& name_token = stream.ConsumeIncludingWhitespace();
  if (name_token.GetType() != kIdentToken) {
    return nullptr;  // Parse error; @counter-style name must be an identifier
  }
  
  AtomicString counter_style_name = name_token.Value().ToAtomicString();
  
  // Check if prelude now contains only the name
  if (!IsAtRulePreludeEmpty(stream)) {
    return nullptr;  // Parse error; prelude should contain exactly one token
  }
  
  // Consume the rest of the prelude
  ConsumeAtRulePrelude(stream);
  size_t prelude_offset_end = stream.LookAheadOffset();
  if (!ConsumeEndOfPreludeForAtRuleWithBlock(stream)) {
    return nullptr;
...
std::shared_ptr<StyleRuleKeyframes> CSSParserImpl::ConsumeKeyframesRule(bool webkit_prefixed,
  ...
  stream.ConsumeWhitespace();
  if (IsAtRulePreludeEmpty(stream)) {
    return nullptr;  // Parse error; prelude should contain keyframes name
  }
  
  // Get keyframes name from stream
  const CSSParserToken& name_token = stream.ConsumeIncludingWhitespace();
...
std::shared_ptr<StyleRuleBase> CSSParserImpl::ConsumeAtRuleContents(
    ...
    CSSNestingType nesting_type,
    std::shared_ptr<const StyleRule> parent_rule_for_nesting) {
  if (allowed_rules == kNestedGroupRulesType) {
  ...
  // prelude.
  String import_prelude_uri;
  if (allowed_rules <= kAllowImportRulesType && id == CSSAtRuleID::kCSSAtRuleImport) {
    import_prelude_uri = ConsumeStringOrURI(stream);
  }

  if (allowed_rules == kKeyframeRulesType || allowed_rules == kNoRulesType) {
  ...
  } else if (allowed_rules <= kAllowImportRulesType && id == CSSAtRuleID::kCSSAtRuleImport) {
    return ConsumeImportRule(std::move(import_prelude_uri), stream);
  } else {
    DCHECK_LE(allowed_rules, kRegularRulesType);
...
std::shared_ptr<StyleRule> CSSParserImpl::ConsumeStyleRule(CSSParserTokenStream& stream,
  ...
  tcb::span<CSSSelector> selector_vector = CSSSelectorParser::ConsumeSelector(
      stream, context_, nesting_type, std::move(parent_rule_for_nesting),
      semicolon_aborts_nested_selector, style_sheet_, nullptr, arena_, &has_visited_pseudo);
...
StyleRuleBase::LayerName ConsumeCascadeLayerName(CSSParserTokenRange& range) {
  CSSParserTokenRange checkpoint = range;
  StyleRuleBase::LayerName name;
  while (!range.AtEnd() && range.Peek().GetType() == kIdentToken) {
    const CSSParserToken& name_part = range.Consume();
    name.emplace_back(name_part.Value().ToAtomicString());

    // Check if we have a delimiter '.' followed by an identifier
    bool has_next_part = false;
    if (range.Peek().GetType() == kDelimiterToken && range.Peek().Delimiter() == '.') {
      // Temporarily consume the '.' to check what's next
      CSSParserTokenRange dot_checkpoint = range;
      range.Consume(); // consume the '.'
      has_next_part = range.Peek().GetType() == kIdentToken;
      range = dot_checkpoint; // restore to before the '.'
    }

    if (has_next_part) {
      range.Consume(); // consume the '.'
    } else {
      // No more parts, so consume any trailing whitespace and return
...
StyleRuleBase::LayerName ConsumeCascadeLayerName(CSSParserTokenStream& stream) {
  stream.EnsureLookAhead();
  CSSParserTokenStream::State checkpoint = stream.Save();
  StyleRuleBase::LayerName name;
  while (!stream.AtEnd() && stream.Peek().GetType() == kIdentToken) {
    ...
    if (stream.Peek().GetType() == kDelimiterToken && stream.Peek().Delimiter() == '.') {
      // Temporarily consume the '.' to check what's next
      stream.EnsureLookAhead();
      CSSParserTokenStream::State dot_checkpoint = stream.Save();
      stream.Consume(); // consume the '.'
      has_next_part = stream.Peek().GetType() == kIdentToken;
      stream.Restore(dot_checkpoint); // restore to before the '.'
    }
    if (!has_next_part) {
      break;
    }
...
std::shared_ptr<StyleRuleImport> CSSParserImpl::ConsumeImportRule(const String& prelude_uri,
                                                                  CSSParserTokenStream& stream) {
  size_t prelude_offset_start = stream.LookAheadOffset();
  CSSParserTokenRange prelude = ConsumeAtRulePrelude(stream);
  size_t prelude_offset_end = stream.LookAheadOffset();
  ...
  if (prelude.Peek().GetType() == kIdentToken && prelude.Peek().Id() == CSSValueID::kLayer) {
    ...
    layer = StyleRuleBase::LayerName{AtomicString::CreateFromUTF8("")};
  } else if (prelude.Peek().GetType() == kFunctionToken && prelude.Peek().FunctionId() == CSSValueID::kLayer) {
    // We need to convert the range to a stream temporarily to use modern parsing
    String layer_string = prelude.Serialize();
    CSSTokenizer layer_tokenizer(layer_string);
    CSSParserTokenStream layer_stream(layer_tokenizer);
    
    if (layer_stream.Peek().GetType() == kFunctionToken && layer_stream.Peek().FunctionId() == CSSValueID::kLayer) {
      CSSParserTokenStream::RestoringBlockGuard guard(layer_stream);
      layer_stream.ConsumeWhitespace();
      StyleRuleBase::LayerName name = ConsumeCascadeLayerName(layer_stream);
      if (name.size() && layer_stream.AtEnd()) {
        layer = std::move(name);
        guard.Release();
        // Consume the function from the original prelude
        prelude.ConsumeBlock();
  ...
  if (prelude.Peek().GetType() == kFunctionToken && prelude.Peek().FunctionId() == CSSValueID::kSupports) {
    // Consume the function and get its contents
...
std::shared_ptr<StyleRuleCharset> CSSParserImpl::ConsumeCharsetRule(CSSParserTokenStream& stream) {
  ...
  stream.ConsumeWhitespace();
  if (IsAtRulePreludeEmpty(stream)) {
    return nullptr;  // Parse error; prelude should contain charset string
  }
  
  // Get charset string from stream
  const CSSParserToken& string_token = stream.ConsumeIncludingWhitespace();
  if (string_token.GetType() != kStringToken) {
    return nullptr;  // Parse error; @charset rule must contain a string
  }
  
  // Check if prelude now contains only the string
  if (!IsAtRulePreludeEmpty(stream)) {
    return nullptr;  // Parse error; prelude should contain exactly one token
  }
...
void CSSParserImpl::ParseDeclarationListForInspector(const String& declaration,
                                                     std::shared_ptr<const CSSParserContext> context,
                                                     webf::CSSParserObserver& observer) {
  CSSParserImpl parser(context);
  parser.observer_ = &observer;
...
std::shared_ptr<StyleRule> CSSParserImpl::CreateImplicitNestedRule(
  ...
  switch (nesting_type) {
    ...
    case CSSNestingType::kScope: {
      ...
      selectors.push_back(selector);
      selectors.push_back(CSSSelector(AtomicString::CreateFromUTF8("scope"), kNotImplicit));
      selectors.back().SetSignal(signal);
      break;
    }
  }
...
std::shared_ptr<StyleRuleBase> CSSParserImpl::ConsumeLayerRule(
  ...
  prelude.ConsumeWhitespace();
  if (prelude.AtEnd()) {
    name.push_back(AtomicString());  // Empty AtomicString for anonymous layer
  } else {
